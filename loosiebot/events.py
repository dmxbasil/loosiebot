...
from loosiebot import MONGO_DB_URI
from loosiebot import telethn

client = MongoClient()
client = MongoClient(MONGO_DB_URI)
db = client["jagannadhuserbot"]
gbanned = db.gban
...
def load_module(shortname):
    if shortname.startswith("__"):
        ...
        import importlib
        import loosiebot.events

        path = Path(f"loosiebot/modules/{shortname}.py")
        name = "loosiebot.modules.{}".format(shortname)
        ...
        mod.loosiebot = loosiebot
        mod.tbot = telethn
        mod.logger = logging.getLogger(shortname)
        spec.loader.exec_module(mod)
        sys.modules["loosiebot.modules." + shortname] = mod
        print("Successfully imported " + shortname)


path = "loosiebot/modules/*.py"
