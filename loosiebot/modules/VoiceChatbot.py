...
from loosiebot import pbot as LYCIA
...
@LYCIA.on_message(filters.command("vai"))
async def Lycia(_, message):
    if len(message.command) < 2:
        await message.reply_text("Loosie AI Voice Chatbot")
        return
    text = message.text.split(None, 1)[1]
    lycia = text.replace(" ", "%20")
    m = await message.reply_text("Loosie Is Best...")
    try:
        ...
        await m.edit(str(e))
        return
    await m.edit("Made By @Iratta_ChunkanRc....")
